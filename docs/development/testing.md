...
message, a colored name, role badges, an image emote, a `/me` action, a cheer, a
sub, a raid, a Super Chat, a Super Sticker, a membership, a system row, and a
live deletion — into the SSE feed, one event at a time:
...
Open the viewer (`/`) or the OBS overlay (`/overlay`) first so you can watch the
events arrive. The demo also flips two throwaway channels (`demo_twitch`,
`@demo_yt`) to `live` in the sidebar and, partway through (after the Twitch
events, before the YouTube ones), deletes one of the earlier messages so you can
see the moderation path.
